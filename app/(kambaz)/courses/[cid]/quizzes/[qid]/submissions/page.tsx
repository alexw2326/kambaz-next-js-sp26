...
export default function QuizPreview({ quiz, time }: { quiz: any; time: string }) {
    ...
    const handleExit = () => {
        const lastSubmission = prevSubmission[prevSubmission.length - 1];
        router.push(`/courses/${cid}/quizzes/${qid}/submissions/${lastSubmission._id}`);
    }
    const handleCreateSubmission = async () => {
        ...
        router.push(`/courses/${cid}/quizzes/${qid}/submissions/${created._id}`);
    }
    ...
    return (
        <div>
            {adminPermission || !findAttempts() ? (
                <div>
                    ...
                    <div>
                        ...
                        <div className="d-flex gap-2 justify-content-end w-75 p-3">
                            ...
                            {isLast ? (
                                <Button variant="success" onClick={handleCreateSubmission} disabled={answers.length !== questions.length}>
                                    Submit Quiz
                                </Button>
                            ) : (
                ...
                <div>
                    <h4>Out of attempts</h4>
                    <Button onClick={handleExit}>See most recent attempt</Button>
                </div>
            )}
        </div>
    );
